		...
		function render_html( $username, $numbers, $cache_time = 1200, $follow = true ) {
			...
			if ( is_null( $pins ) ) {
				...
				foreach ( $pins as $pin ) {
					...
					echo '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" target="_blank" alt="' . esc_attr( $title ) . '">'. $lazyhtml .'</a>';
				}
			}
			?>
			</div>
			<?php if ( $follow ): ?>
				<div class="pin_link">
					<a href="http://pinterest.com/<?php echo sanitize_text_field( $username ); ?>/" rel="nofollow" target="_blank">@<?php echo sanitize_text_field( $username ); ?></a>
				</div>
			<?php endif; ?>
		<?php
		}
